...
export default function Hero() {
  ...
  return (
    <section id="top" className="relative w-full flex items-center justify-center min-h-[480px] sm:min-h-[560px] md:min-h-[640px] overflow-hidden py-12 sm:py-16 md:py-20">
      <img
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center text-center px-4 sm:px-6">
        <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs sm:text-sm md:text-base font-semibold text-white bg-white/20 backdrop-blur rounded-full mb-4 sm:mb-6 border border-accent-300/40">
          <i className="ri-shield-check-line text-accent-300"></i>
          {t("hero.badge", "개인정보 없이 · 안심하고 나누는 상담")}
        </span>

        <h1 className="font-heading text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight max-w-4xl drop-shadow-[0_4px_16px_rgba(0,0,0,0.7)] break-keep">
          {t("hero.title1", "어려움이 있어도,")}
          <br />
          {t("hero.title2", "혼자 고민하지 마세요")}
        </h1>

        <p className="mt-4 sm:mt-6 text-sm sm:text-base md:text-xl text-white/95 max-w-3xl leading-relaxed break-keep">
        ...
        <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
          <button
            type="button"
            onClick={() => openChat()}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 text-base sm:text-lg font-bold rounded-full bg-primary-500 text-white hover:bg-primary-600 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg whitespace-nowrap"
          >
          ...
          <Link
            to="/guide"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 text-base sm:text-lg font-semibold rounded-full bg-white/20 text-white backdrop-blur hover:bg-white/30 transition-colors whitespace-nowrap"
          >
