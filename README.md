...
> 공공데이터포털(data.go.kr) 공식 API 및 지자체 고시 기반 4단계 맞춤 복지/생활 지원 솔루션
...
1. **지능형 질문 라우팅 (Smart Routing Engine)**
   ...
   - **복합 위기 상담**: 공공데이터 원문 기반 4단계 맞춤형 지원 계획 생성 + **[이 서비스에 도움 요청하기]** 연계.
...
3. **어르신 친화 기능 (Elder-Friendly UI/UX)**
   ...
   - 맞춤 지원 전체 내러티브 음성 안내 및 개별 스텝/챗봇 답변 음성 듣기 (TTS)
   - 히든 Iframe 격리 기법을 적용한 **A4 맞춤 안내서 1장 인쇄 엔진**

4. **신뢰성 및 안전성 (3대 절대 원칙 준수)**
   - **1) 개인정보 절대 수집 금지**: 주민번호/실명/계좌번호 일체 미보관
   - **2) 검증된 공공데이터 기반**: 허위 정보(Hallucination) 방지 및 공식 출처 명시
   - **3) 쉬운 우리말 설명**: 어르신 눈높이에 맞춘 일상 언어 변환
